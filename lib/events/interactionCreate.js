    ...
    async execute(interaction) {
        ...
        if (interaction.isAutocomplete() || interaction.isChatInputCommand()) {
            ...
            if (interaction.isAutocomplete()) {
                if (!command?.enabled) 
                    return interaction.respond([ { name: responses.interactions.errors.autocomplete.AutocompleteInteractionDisabledMessage, value: "ERR_COMMAND_DISABLED" } ]);
                if (command?.restricted && !checkPermsLevel(command)) 
                    return interaction.respond([ { name: responses.interactions.errors.autocomplete.AutoCompleteInteractionRestrictedMessage, value: "ERR_INSUFFICIENT_PERMISSION" } ]);
                try {
        ...
        if (interaction.isModalSubmit()) { // respond to modals here!
            switch (interaction.customId) {
                case "MODAL_CUSTOM_ID":
                    break;
                default:
                    logger.warn(`Unknown modal with ID "${interaction.customId}" received!`);
            };
        };
    }
};
