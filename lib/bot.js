...
for (const filePath of readdirSync(cmdRootDir, { recursive: true }).filter((item) => item.endsWith(".js"))) {
    const cmdFilePath = path.join(cmdRootDir, filePath);
    try {
        const command = require(cmdFilePath);
        logger.verbose(JSON.stringify(command)); // verbose debug output for commands
        if ("data" in command && "execute" in command) {
            // validate imported command before adding to collection
            if (typeof command.execute !== "function") // check if "command.execute" is a function.
                throw TypeError(`Expected "execute" to be a callback function, got ${typeof command.execute} instead!`);
            if ("autocomplete" in command && typeof command?.autocomplete !== "function") // check if "command.autocomplete" exists and is a function.
                throw TypeError(`Expected "autocomplete" to be a callback function, got ${typeof command?.autocomplete} instead!`);
            command.path = cmdFilePath; // add command file path reference for reloading functionality
            client.commands.cache.set(command.data.name, command); // add command to bot client's commands cache
            logger.debug(`Loaded command.${command.data.name} from file "${command.path ?? cmdFilePath}"`);
        } else {
            const reason = () => {
                const reason = () => {
