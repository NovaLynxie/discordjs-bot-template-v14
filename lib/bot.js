...
for (const cmdSubDir of readdirSync(cmdRootDir)) {
    const cmdDirPath = path.join(cmdRootDir, cmdSubDir);
    const cmdFiles = readdirSync(cmdDirPath).filter((file) => file.endsWith(".js"));
    for (const file of cmdFiles) {
        const cmdFilePath = path.join(cmdDirPath, file);
        try {
